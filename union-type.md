...
またユニオン型が上記の`prim`、`creature`のように2択で、`typeof`、`instanceof`の`if`のブロック内で`return`が行われれば、`if`のブロックより下ではもう片方の型であると自動的に型を確定してくれます。上記例8行目の`prim`がそれにあたります。だと`typeof`で`number`型と確定した`if`ブロックは5行目で`return`をしているので、7行目以降では`prim`はもう片方の型である`string`型であることが確定し、入力補完候補も`string`型のものが表示されるようになります。
