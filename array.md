...
配列を`[]`という`Sugar syntax`で書くことができるのはインスタンスに限ります。`static method`を呼びたい時は`Array`と書かなければいけません。例えば反復可能であることを示すインターフェイスの`Iterable<T>`から配列を作成する`Array.from<T>()`は`[].from<T>`と書くことはできず、必ず`Array`と書かなければいけません。
