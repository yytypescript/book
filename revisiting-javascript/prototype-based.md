...
一方のJavaScriptのオブジェクト生成はプロトタイプベースです。プロトタイプベースの特徴は、クラスのようなものが無いこところです。\(あったとしてもクラスもオブジェクトの一種だったりと特別扱いされていない\)
...
JavaScriptが採用しているプロトタイプベースがどういうものなのか見てきました。では、なぜJavaScriptはクラスベースではなくプロトタイプベースを選んだのでしょうか。プロトタイプベースにした狙いとは何だったのでしょうか。

JavaScriptが開発には次のような要件がありました。ブラウザで動く言語で、構文はJava風に。しかし、Javaほど大掛かりでないようにと。そして、開発期間はというと、10日と逼迫したものでした。
...
これはあくまでSelfの意見でJavaScriptが明言したわけではありませんが、歴史の文脈から読み取るに、JavaScriptもSelfの考え方に共感してプロトタイプベースを採用のは明らかです。JavaScriptのプロトタイプベース採用の背景には、言語をシンプルで柔軟なものにしたいという考えが根底にあったわけです。
...
* プロトタイプベースでの継承は、特別な操作ではなく、オブジェクト生成と全く同じプロセスである。
