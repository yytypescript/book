...
```sh
npm create vite@latest like-button -- --template react-ts --no-interactive
```
...
```taml
  <bold><green>VITE</green></bold><green> v8.0.8</green>  <dim>ready in </dim>223 ms
...
![「Get started」見出しとカウントボタンが表示されたVite + Reactスターターページ](react-like-button-tutorial/screen1.png)
...
ひながたにはデフォルトのスタイルシートが含まれていますが、このチュートリアルでは使いません。`src/main.tsx`を開き、`import './index.css'`の行を削除してください。

```tsx twoslash title="src/main.tsx"
// @jsx: react-jsx
// @moduleResolution: bundler
// @noEmit: true
// @allowImportingTsExtensions: true
// @filename: react-dom-client.d.ts
...
次に、`src/App.css`の内容を次のように書き換えてください。画面中央にコンテンツを配置するための最小限のスタイルです。
