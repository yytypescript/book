...
このチュートリアルでは、テストフレームワーク「Jest」を使い、ReactコンポーネントのテストをTypeScriptで書くことを学びます。
...
Reactコンポーネントをテストする方法は複数ありますが、ここでは利用者が比較的多い`testing-library`というライブラリパッケージを用います。
`testing-library`はUIコンポーネントのテストをするためのライブラリパッケージであり、コンポーネントに対する操作やコンポーネントの描画などが実現できます。`testing-library`があれば、コンポーネントのテストはひととおりできると考えてよいです。
