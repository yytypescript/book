...
型注釈の部分はTypeScript固有のものです。それが書いてあるとブラウザやサーバーサイドのJavaScript実行環境では実行できません。なので、TypeScriptコンパイラでJavaScript実行環境で動かす用のJavaScriptファイルを生成します。開発者はこの成果物のJavaScriptファイルを本番環境にデプロイすることになります。

このように、TypeScriptで書き、JavaScriptに変換し、デプロイするという流れが基本ではあるものの、実務ではJavaScriptに変換する手順が自動化されていることが多いです。たとえば、Next.jsなどのフレームワークやViteなどバンドラーを用いているプロジェクトでは、`tsc`コマンドの出番はチェックくらいしかなく、JavaScriptへの変換はビルドプロセスに隠蔽されています。サーバーサイドJavaScript環境のNode.jsの最近のバージョンでは、直接TypeScriptを実行できるようになっています。このように、JavaScriptへの変換をあまり意識しなくてもいいように、エコシステムが充実してきている点も知っておくとよいでしょう。
