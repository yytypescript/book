...
`keyof`はobject型からプロパティ名を型として返す型演算子です。たとえば、`name`プロパティを持つ型に対して、`keyof`を使うと文字列リテラル型の`"name"`が得られます。
...
2つ以上のプロパティがあるobject型に`keyof`を使った場合は、すべてのプロパティ名がユニオン型で返されます。
...
プロパティを持たないobject型に`keyof`を使うと`never`型が返ります。
...
`keyof`のメリットは、保守性が上がる点です。object型とは別にプロパティ名のユニオン型を定義していると、object型のプロパティを変更したときに、そのユニオン型のほうも修正が必要になります。`keyof`を使って、object型からキーを導出するようにしておけば、変更箇所はobject型のところだけになります。
