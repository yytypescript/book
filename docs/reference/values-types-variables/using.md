...
メモリ領域以外のリソースの管理はGCは行わないため、非メモリリソースの管理はプログラマーが明示的に行なう必要があります。例えば、以下の Deno 環境で作成されたファイル読み込みの処理では、ファイルハンドルは以下のように読み取りのために `open()` したら、利用終了時には `close()` するという処理を行なう必要があります。
