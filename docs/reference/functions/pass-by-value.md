...
関数の実引数に変数を指定するとき、仮引数への渡し方に値渡し(pass-by-value)と参照渡し(pass-by-reference)の 2 種類があります（値渡しはさらに細かく分けられます）。
...
次のコードは、C++言語の値渡しの例です。変数`a`は`1`で初期化され、関数`change`に変数`a`の値が渡されます。関数`change`内で仮引数`n`に`2`を代入しても、呼び出し元の変数`a`は`1`のままです。
