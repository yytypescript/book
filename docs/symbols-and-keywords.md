...
ここで扱う記号とキーワードには、JavaScript由来のもの、つまり、JavaScriptとTypeScriptに共通して使えるものと、TypeScriptでのみ使えるものを併記しています。JavaScript由来のものには![js](assets/js.svg)のマークを、TypeScript固有のものには![TS](assets/ts.svg)マークを表示しています。

## 記号とキーワード

### `!` 論理否定演算子 (logical not operator) ![js](assets/js.svg)

真値と偽値を反転します。

### `!` 非Nullアサーション (non-null assertion operator) ![ts](assets/ts.svg)
...
### `!` Definite Assignment Assertion演算子 ![ts](assets/ts.svg)
...
### `!==` 厳密不等価演算子 (strict inequality operator) ![js](assets/js.svg)
...
### `"` 文字列リテラル (string literal) ![js](assets/js.svg)

`"foo"`のように文字列リテラルの開始と終了に使われる記号です。

### `#` プライベートプロパティ (private property) ![js](assets/js.svg)
...
### `%` 剰余演算子 (reminder operator) ![js](assets/js.svg)
...
### `%=` 剰余代入 (reminder assignment) ![js](assets/js.svg)

左の変数の値に右の値で割り算した余りを左の変数に割り当てます。

### `&` ビット論理積 (bitwise and) ![js](assets/js.svg)
...
### `&=` ビット論理積代入 (bitwise and assignment) ![js](assets/js.svg)
...
### `&&` 論理積 (logical and) ![js](assets/js.svg)

すべての真偽値が `true` のときに `true` を返します。そうでない場合に `false` を返します。

### `&&=` 論理積代入 (logical and assignment) ![js](assets/js.svg)
...
### `()` 即時実行関数の一部 (IIFE: immediately invoked function expression) ![js](assets/js.svg)
...
### `*` 乗算演算子 (multiplication operator) ![js](assets/js.svg)

左の値と右の値を掛け算します。

### `*` ジェネレーター関数の宣言 (generator) ![js](assets/js.svg)
...
### `*` yield\*式 (yield) ![js](assets/js.svg)
...
### `*=` 乗算代入 (multiplication assignment) ![js](assets/js.svg)

左の変数の値と右の値を掛け算した結果を左の変数に割り当てます。

### `**` べき乗演算子 (exponentiation) ![js](assets/js.svg)
...
### `**=` べき乗代入 (exponentiation assignment) ![js](assets/js.svg)

左の変数の値を右の値でべき乗した結果を左の変数に割り当てます。

### `+` 単項正値演算子 ![js](assets/js.svg)
...
### `+` 加算演算子 (addition operator) ![js](assets/js.svg)

2つの値を足し算します。

### `+` 文字列結合演算子 (concatenation operator) ![js](assets/js.svg)

2つの文字列を結合します。

### `+` 修飾子の付加 ![ts](assets/ts.svg)
...
### `+=` 加算代入 (addition assignment) ![js](assets/js.svg)

左の変数の値とに右の値を足し算した結果を左の変数に割り当てます。

### `++` インクリメント (increment) ![js](assets/js.svg)
...
### `,` オブジェクトプロパティの区切り ![js](assets/js.svg)
...
### `,` カンマ演算子 (comma operator) ![js](assets/js.svg)
...
### `-` 減算演算子 (subtraction operator) ![js](assets/js.svg)

2つの値を引き算します。

### `-` 修飾子の削除 ![ts](assets/ts.svg)
...
### `-=` 減算代入 (subtraction assignment) ![js](assets/js.svg)

左の変数の値から右の値を引き算した結果を左の変数に割り当てます。

### `--` デクリメント (decrement) ![js](assets/js.svg)
...
### `.` プロパティへのアクセス (dot operator) ![js](assets/js.svg)
...
### `...` スプレッド構文 (spread syntax) ![js](assets/js.svg)
...
### `...` 残余構文 (rest syntax) ![js](assets/js.svg)
...
### `/` 除算演算子 (division operator) ![js](assets/js.svg)

左の値を右の値で割り算します。

### `/` 正規表現リテラル (regular expression literal) ![js](assets/js.svg)

`/[0-9]+/`のような正規表現リテラルの前後に書かれる記号です。

### `/=` 除算代入 (division assignment) ![js](assets/js.svg)

左の変数の値を右の値で割り算した結果を左の変数に割り当てます。

### `//` 一行コメント (one line comment) ![js](assets/js.svg)

行コメントの開始を表す記号です。

### `/*` 複数行コメント (multiline comment) ![js](assets/js.svg)
...
### `:` 三項演算子の一部 (conditional operator) ![js](assets/js.svg)

`a ? b : c`のような三項演算子のelseを表すのに用いられる記号です。

### `:` 型アノテーション ![ts](assets/ts.svg)
...
### `<` 小なり演算子 (less than operator) ![js](assets/js.svg)

左の値が右の値よりも小さいか判定します。

### `<` ジェネリクス ![ts](assets/ts.svg)
...
### `<=` 小なりイコール演算子 (less than or equal) ![js](assets/js.svg)

左の値が右の値以下か判定します。

### `<<` ビット左シフト演算子 (left shift operator) ![js](assets/js.svg)
...
### `<<=` 左シフト代入 (left shift assignment) ![js](assets/js.svg)
...
### `=` 代入演算子 (assignment) ![js](assets/js.svg)

左の変数に右の値を割り当てます。

### `==` 等価演算子 (equality) ![js](assets/js.svg)
...
### `===` 厳密等価演算子 (strict equality) ![js](assets/js.svg)
...
### `=>` アロー関数の一部 (arrow function) ![js](assets/js.svg)
...
### `>` 大なり演算子 (greater than) ![js](assets/js.svg)

左の値が右の値よりも大きいか判定します。

### `>=` 大なりイコール演算子 (greater than or equal) ![js](assets/js.svg)

左の値が右の値以上か判定します。

### `>>` ビット右シフト演算子 (right shift) ![js](assets/js.svg)
...
### `>>=` 右シフト代入 (right shift assignment) ![js](assets/js.svg)

左の変数の値のビットを右の値の数だけ右にずらした結果を左の変数に割り当てます。

### `>>>` 符号なし右シフト演算子 (unsigned right shift) ![js](assets/js.svg)
...
### `>>>=` 符号なし右シフト代入 (unsigned right shift assignment) ![js](assets/js.svg)
...
### `?.` オプショナルチェーン (optional chaining) ![js](assets/js.svg)
...
### `??` Null合体 (nullish coalescing operator) ![js](assets/js.svg)
...
### `??=` Null合体代入 (logical nullish assignment) ![js](assets/js.svg)
...
### `@` デコレーター ![ts](assets/ts.svg)

デコレーターはクラスやクラスメンバーに注釈を追加するもので、デコレーターを使うのに用いられる記号です。

### `[` 配列リテラル (array literal notation) ![js](assets/js.svg)

`[1, 2, 3]`のような配列リテラルの開始に用いられる記号です。

### `[` アクセッサー (bracket notation) ![js](assets/js.svg)
...
### `[` 配列の分割代入 (destructuring assignment) ![js](assets/js.svg)
...
### `\` 文字列エスケープシーケンス (escaping character) ![js](assets/js.svg)
...
### `^` ビット排他的論理和 (bitwise xor) ![js](assets/js.svg)
...
### `^=` ビット排他的論理和代入 (bitwise xor assignment) ![js](assets/js.svg)

左の変数の値と右の値でビットの値が異なる位置のビットを1にした結果を左の変数に割り当てます。

### `_` 数値の区切り文字 ![js](assets/js.svg)
...
### `` ` `` テンプレートリテラル (template literal) ![js](assets/js.svg)
...
### `as` 型アサーション ![](assets/ts.svg)

TypeScriptコンパイラーが解釈した型を上書きする「型アサーション」に用いられるキーワードです。

### `is` 型アサーション関数の一部 ![](assets/ts.svg)
...
### `keyof` keyof型演算子 ![](assets/ts.svg)

オブジェクト型からプロパティ名を型として返す型演算子です。

### `n` bigintリテラル ![js](assets/js.svg)
...
### `typeof` typeof演算子 ![js](assets/js.svg)
...
### `typeof` typeof型演算子 ![](assets/ts.svg)
