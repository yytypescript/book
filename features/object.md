...
そこでオブジェクトの型を独自に定義することができます。独自に定義した肩にエイリアス\(別名\)をつけて使い回すこともできます。この機能については本書のタイプエイリアスの項を参照ください。
...
ただし、インターフェイスはオブジェクトの中にあるメソッドを定義することはできます。
...
`{}`はメンバーを持たないオブジェクトリテラルの型定義です。こちらも`object`と同様に入力補完するプロパティ、メソッドはありません。
...
文字数カウントをしたい時は`str.length`とすれば文字数がえられます。また、数値を文字列にしたければ\(`backtick`などを使わなければ\)`num.toString()`とすれば文字列がえられます。

プリミティブ値はオブジェクトではないのでプロパティやメソッドを持っていないはずですがこのようなことができるのは、内部的にはJavaScriptがプリミティブ値をオブジェクトに変換しているからです。この暗黙の型変換を`Autoboxing`と言います。
...
継承関係にないクラスでも、プロパティとメソッドが一致さえすれば代入ができてしまいます。これは公称型\(`nominal typing`\)を型のシステムとして使っている言語にはない特徴です。
